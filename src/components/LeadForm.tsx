...
export function LeadForm() {
  ...
  return <motion.form id="consult" onSubmit={onSubmit} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="rounded-3xl border border-slate-200 bg-white/85 p-6 shadow-soft backdrop-blur dark:border-white/10 dark:bg-slate-900/80">
    ...
    <div className="mt-5 grid gap-4">
      <label className="text-sm font-medium">Имя<input value={form.name} onChange={e=>setForm({...form,name:e.target.value})} className="mt-1 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-indigo-500 dark:border-white/10 dark:bg-slate-950" placeholder="Ваше имя" /></label>
      {errors.name && <span className="text-sm text-rose-500">{errors.name}</span>}
      <label className="text-sm font-medium">Телефон<input value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} className="mt-1 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-indigo-500 dark:border-white/10 dark:bg-slate-950" placeholder="+7 900 000-00-00" /></label>
      {errors.phone && <span className="text-sm text-rose-500">{errors.phone}</span>}
      <label className="text-sm font-medium">Для кого курс<select value={form.audience} onChange={e=>setForm({...form,audience:e.target.value})} className="mt-1 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none dark:border-white/10 dark:bg-slate-950"><option>Подросток 14–18</option><option>Студент</option><option>Взрослый</option></select></label>
      <label className="text-sm font-medium">Комментарий<textarea value={form.message} onChange={e=>setForm({...form,message:e.target.value})} className="mt-1 min-h-24 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none dark:border-white/10 dark:bg-slate-950" placeholder="Что хотите прояснить?" /></label>
